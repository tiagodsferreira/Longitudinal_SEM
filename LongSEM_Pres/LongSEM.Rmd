...
::: gray2
- **Análise Fatorial Confirmatória (CFA) com dados longitudinais**  
...
A CFA permite avaliar se um determinado modelo de medida teórico se ajusta a um conjunto de dados  

- O modelo CFA representa as relações entre uma ou mais variáveis latentes ou fatores e os seus indicadores observados   
  - A CFA foi abordada anteriormente com dados transversais
  
```{r, echo=FALSE, fig.width=10, fig.height=7, out.width="40%"}
knitr::include_graphics("G:\\My Drive\\FPCEUP\\AtividadeDocente\\ConfirmatoryFactorAnalysis\\files\\figures\\cfamodel.png")
```

### A CFA longitudinais é apenas uma extensão deste modelo
s


## CFA Multigrupo
### CFA multigrupo permite avaliar o ajustamento de modelo de medida a diferentes subgrupos da população (ex.: género, coortes, contextos culturais)
  
### A CFA multigrupo pode ser generalizada a longitudinais, correspondendo os grupos a diferentes momentos de avaliação\

```{r, echo=FALSE, fig.width=10, fig.height=7, out.width="70%"}
...
## Modelo CFA longitudinal
A seguinte sintaxe especifica o modelo no lavaan:  
...
### Geralmente conduzida por meio de comparação de modelos encaixados
...
### Teste formal: Likelihood Ratio Test (**LRT**) para modelos encaixados
...
### Garante que **mudanças nos valores da variável latentes** refletem mudanças reais no construto *e não alterações na forma como o construto é medido*
...
- **Estes níveis são comparados sequencialmente** 
  - Primeiro modelo livre (configural)\
  - Depois adicionam-se restrições métricas, escalares, etc.\
  - Avaliamos mudança no ajuste do modelo: $\Delta{CFI}$, $\Delta{RMSEA}$, $\Delta\chi^2$\

### Ferramentas comuns: lavaan em R; Mplus; AMOS
...
### Definindo explícita dos modelos (recomendada)
...
### Através da função **measEq.syntax()** do pacote semTools
...
all_params <- parameterEstimates(fit.scalar, standardized = TRUE, rsquare = TRUE)
...
all_params <- parameterEstimates(fit.strict, standardized = TRUE, rsquare = TRUE)
head(all_params[all_params$op == "~~", ])
```
...
Estes modelos procuram explicar uma variável em função do tempo, descrevendo:  
  - Nível inicial e trajetória de mudança médias dos indivíduos  
